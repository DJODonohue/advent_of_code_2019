...
/* This whole method was wrong and a bad idea

function wire_direction(wire){
    for (entry of wire) {
      console.log(entry[0]);
    }
}
//wire_direction(wire_1);
//wire_direction(wire_2);

function distance(wire){
  for (entry of wire) {
    console.log(entry.substring(1));
  }
}
...
function get_y_coordinates(wire) {
  let y = 0;
  let y_coordinates = []; //this 0 is either 100% necessary or the whole problem //GUESS WHAT IT WAS SOMETHING COMPLETELY DIFFERENT
  for (entry of wire) {
    if (entry[0] == "U") {
